...
- **Machine-parsed block headings — block on the FACT, not on a belief about the parser**: \`## Harvested Allocations\` / \`## Derived Values\` / \`## Consumed Values\` must exist as heading lines with exactly that text (any heading level; emphasis furniture and a section number such as \`6.\` tolerated). A RETITLED, MERGED or absent marker is a blocking defect — the checker reads it ABSENT. A correctly-titled heading NESTED under another section is NOT (heading-tolerant parser since 2026-07-18); flag it as a non-blocking observation. Never state what "the platform's checker reads" as your own finding — the platform stamps that fact itself (\`derivationContainment\` on the leg's card) and it outranks your reading (devext Run 3, 2026-09-09: a parsed-clean block was blocked on this belief).
