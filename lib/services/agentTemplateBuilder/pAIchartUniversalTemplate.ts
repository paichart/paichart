...
- Common mistakes to avoid: (1) writing generic industry commentary when POV context specifies a narrower scope — the output must be about THIS customer's situation, not "typical US hospital networks"; (2) inventing data (e.g., "the average breach costs $5.9M") without citing the source and marking it as an external benchmark rather than a POV-specific finding`,
   ...
   * was caught in review. (D9 — DISCHARGED 2026-09-12, all five parts in ONE commit: the slot below is abstracted to an
   * APPLY-GOVERNANCE note, observability's two paper-over clauses are retired, the two silent domains
   * (terraform-iac, kubernetes-gitops) state the property, all eight rows reseeded, and the four-domain
   * live dry-run is the acceptance gate.
   * The term is OBSERVABILITY'S OWN — it had already invented the right abstraction locally and then
   * papered over this slot with it; promoting a proven in-domain term beat coining a new one.
   * NETWORK KEEPS ITS MAINTENANCE WINDOW: in that domain the apply genuinely is a window-scheduled
   * cutover, so the window is the domain INSTANCE of the property, not a network-ism — which is also
   * what makes network the CONTROL for the dry-run: its behaviour must NOT move.
   * What the measurement found: 44 of 49 non-network archived packages carried a maintenance-window
   * section they had to invent a meaning for (k8s 5/5, terraform 31/33, observability 8/11). The
   * failure was low-stakes fabrication — continuous, invisible, and never blocking anything.
   * The fix shape is CHECK-DESIGN rule 2: convert an ABSENCE into a DECLARED, NAMED state, so
   * "no window needed, because …" is a checkable answer rather than a blank slot an author feels
   * obliged to fill. Same shape as `linksNotApplicable`, three days earlier in the same sweep.
   * Record: cline_docs/reviews/program-tier-domainism-sweep-2026-09-12/MEASUREMENT.md I-1.)
...
- Independently review the change package for: standards/lint, blast-radius, rollback adequacy per target, and approval readiness — including the **apply-governance note**: judge whether it states what the apply requires, what it disrupts while running, and the first post-apply check. A declared "nothing to schedule, because …" is a VALID answer — judge the REASON, not the presence of a window. Never require a scheduled maintenance window in a domain whose apply has no cutover, and treat a timing constraint with no basis in the harvested state, the requirements, or your active protocol as INVENTED — name it as a blocking issue.
...
- **Deliverable**: Structure your final response as: blocking issues you still hold (named, per-target where applicable) → standards/blast-radius/rollback assessment table → apply-governance assessment → the terminal verdict block (LAST — nothing after it).
- **Coordination**: Use \`task.comment\` for status: which package sections reviewed, blockers escalated. Do NOT call \`task.complete\` on your own task — the platform completes it when your execution persists, and a self-completion queues the harness against a snapshot that does not yet contain your verdict (H-6, 2026-09-10). Your FINAL verdict lives in your terminal \`## VERDICT:\` block (e.g. "APPROVED, no blocking issues, confidence 86") — never a superseded concern.
