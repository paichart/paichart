...
function evaluateServiceRegistration(serviceData, context) {
  ...
  // E14 (2026-09-07, panel BC-1): the caller already ran assertEndpointSafe — the authoritative,
  // canonicalising endpoint gate. When it admitted the endpoint via the operator allowlist
  // (HUB_PRIVATE_ENDPOINT_ALLOWLIST), this evaluator must agree, not re-reject on a substring.
  // The former admin-localhost carve-out here was DEAD (it read context.user.role, which no caller
  // passed) and is superseded: reachability is decided by the gate, never by the actor's role.
  const endpointAllowlisted = context?.endpointAllowlisted === true;
  ...
  // 3. Check endpoint for blocked domains
  if (endpointAllowlisted) {
    warnings.push({
      type: 'OPERATOR_ALLOWLISTED_ENDPOINT',
      message: `Private endpoint admitted by the operator allowlist (${context?.endpointAllowlistEntry || 'HUB_PRIVATE_ENDPOINT_ALLOWLIST'})`,
      severity: 'LOW'
    });
    checks.push({ type: 'OPERATOR_ALLOWLIST', message: 'Endpoint reachability decided by HUB_PRIVATE_ENDPOINT_ALLOWLIST', severity: 'INFO' });
  } else {
      ...
      if (endpoint.toLowerCase().includes(domain)) {
        ...
        risks.push({
          ...
          suggestion: isLocalhostDomain
            ? 'Private endpoints are refused unless the operator lists them in HUB_PRIVATE_ENDPOINT_ALLOWLIST (self-host); otherwise use public HTTPS endpoints.'
  ...
  if (criticalRisks > 0) {
    ...
    if (hasLocalhostIssue) {
      userGuidance = [
        ...
        '✅ For a self-hosted hub: the operator lists the address in HUB_PRIVATE_ENDPOINT_ALLOWLIST (docs/RUNNING.md)',
