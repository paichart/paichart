...
// E14 (2026-09-07): connect sites consult the operator allowlist through this wrapper; the pure
// validateUrlSafety stays for the parameter-URL scan in service-call-policy.js (a different question).
const { validateEndpointSafety } = require('../../../../utils/endpoint-allowlist');
...
function assertEndpointSafe(endpoint, { existingService = null, action = 'register' } = {}) {
  if (existingService && isSSRFExemptService(existingService)) return { safe: true, exempt: true };
  const urlCheck = validateEndpointSafety(endpoint);
  if (!urlCheck.safe) {
    throw new Error(
  ...
  // E14: callers may read `allowlisted` / `matchedEntry` (register: gate-9 reconciliation + WARN audit line).
  return urlCheck;
}
        ...
        for (const service of services) {
          ...
          try {
            ...
            if (!isSSRFExemptService(service)) {
              const urlCheck = validateEndpointSafety(healthUrl);
              if (!urlCheck.safe) {
