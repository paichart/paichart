...
class ServiceRegistrationHandler {
  ...
  async handle(args, context) {
    try {
      ...
      const endpointCheck = assertEndpointSafe(validatedArgs.endpoint, { action: 'register' });
      if (endpointCheck?.allowlisted) {
        // E14: operator-declared private endpoint (HUB_PRIVATE_ENDPOINT_ALLOWLIST). Audit fact, register/update only.
        log.warn({ securityEvent: true, serviceName: validatedArgs.name, endpoint: validatedArgs.endpoint, matchedEntry: endpointCheck.matchedEntry, userId }, 'Endpoint admitted by operator allowlist');
      }
      ...
      const serviceEvaluation = evaluateServiceRegistration(validatedArgs, {
        ...
        // E14 (panel BC-1): the approval policy's substring BLOCKED_DOMAINS loop is a second endpoint gate;
        // it must agree with the decision assertEndpointSafe just made or an allowlisted registration is
        // born INACTIVE / PENDING_APPROVAL and services.call refuses it.
        endpointAllowlisted: !!endpointCheck?.allowlisted,
        endpointAllowlistEntry: endpointCheck?.matchedEntry
      });
      ...
      if (!isAutoApproved) {
        // BC-2 (Bug Class 80): the evaluator returns { evaluation: { riskLevel, risks, warnings } } — top-level reads were phantom (undefined)
        log.warn({ securityEvent: true, serviceName: validatedArgs.name, riskLevel: serviceEvaluation.evaluation?.riskLevel, violations: serviceEvaluation.evaluation?.risks, userId }, 'Service requires compliance approval');
        ...
        return {
          ...
          approvalDetails: {
            riskLevel: serviceEvaluation.evaluation?.riskLevel,
            estimatedReviewTime: '24-48 hours',
            reasons: serviceEvaluation.evaluation?.risks || [],
            warnings: serviceEvaluation.evaluation?.warnings || [],
