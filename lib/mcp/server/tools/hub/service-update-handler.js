  ...
  async handle(args, context) {
    try {
      ...
      if (safeUpdates.endpoint) {
        const endpointCheck = assertEndpointSafe(safeUpdates.endpoint, { existingService, action: 'update' });
        if (endpointCheck?.allowlisted) {
          log.warn({ securityEvent: true, serviceName: existingService?.name, endpoint: safeUpdates.endpoint, matchedEntry: endpointCheck.matchedEntry, userId }, 'Endpoint admitted by operator allowlist');
        }
      }
