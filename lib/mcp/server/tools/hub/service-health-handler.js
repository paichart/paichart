...
const { validateEndpointSafety } = require('../../../../utils/endpoint-allowlist'); // E14: connect site (operator allowlist)
      ...
      if (endpoint) {
        try {
          ...
          if (endpoint.startsWith('http://') || endpoint.startsWith('https://')) {
            ...
            const urlCheck = isSSRFExemptService(service) ? { safe: true } : validateEndpointSafety(healthUrl);
            if (!urlCheck.safe) {
