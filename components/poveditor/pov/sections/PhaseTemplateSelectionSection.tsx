...
export default function PhaseTemplateSelectionSection() {
  const { state, updateField, addEntity, reorderRelationship } = useEditorContext();
  ...
  useEffect(() => {
    ...
    const fetchPhaseTemplates = async (retryCount = 0, delay = 1000) => {
      try {
        ...
        if (state.data.phaseTemplateIds && Array.isArray(state.data.phaseTemplateIds)) {
          setSelectedTemplateIds(state.data.phaseTemplateIds);
          phaseTemplateCache.current[cacheKey] = state.data.phaseTemplateIds;

          try {
            localStorage.setItem(`phaseTemplates_${cacheKey}`, JSON.stringify(state.data.phaseTemplateIds));
          } catch {
        ...
        if (selectedTemplateIds.length === 0 && state.data.phaseTemplateIds && Array.isArray(state.data.phaseTemplateIds)) {
          setSelectedTemplateIds(state.data.phaseTemplateIds);
        }
      } catch (err) {
  ...
  // Update state when selected templates change
  useEffect(() => {
    updateField(['data', 'phaseTemplateIds'], selectedTemplateIds);
  }, [selectedTemplateIds, updateField]);
  ...
  }, [state.data.id, state.ui.activeTab]); // React to tab changes and POV ID changes
  ...
  const handleApplyTemplates = async () => {
    ...
    try {
      // The selectedTemplateIds are already updated in the main editor state (state.data.phaseTemplateIds)
      // by the useEffect hook that watches local selectedTemplateIds.
      // This button now primarily serves as a user confirmation step.
      // Actual phase creation from templates will occur server-side when "Save POV" is clicked,
      // handled by ensurePhasesFromTemplates.
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select phase templates to include in your POV
          </CardDescription>
        </CardHeader>
