...
Those checks are not one pass — they run in **three separate tiers, and a lower tier can't be talked out of its answer by a higher one.** First, a **mechanical tier**: plain deterministic code computes the containment, coverage, contract-presence and protocol-completeness facts directly from each leg's own harvested evidence — no model in the loop. Second, **independent reviewers** read the change: one per leg against its own contract, plus a program-level integration reviewer that checks the legs agree *with each other* and runs the domain's *own* validators (`terraform validate`, config parsers, topology analysers) over the *composed* change — it consumes those validators as tools, it doesn't reimplement them. Third, the **release gate** is the deterministic AND over the facts the first two tiers produced.

The layering is the point. A mechanical containment violation blocks the release **regardless of any reviewer approving above it** — a reviewer's "looks fine" is advisory for the arithmetic and never stands in for the containment check. And the gate fails *safe*: a check that couldn't run — because a leg dropped evidence it was required to carry, or the inputs wouldn't parse — counts as a **block, not a pass**. "We couldn't verify it" is never quietly rounded up to "it's fine." That is what stops a single confident wrong answer, at any tier, from carrying a change to release.
