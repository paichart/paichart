...
**Companion documents**:
- For tutorial-style introduction with worked examples: [Chapter 2 — Ten UX + Five Plumbing Standards](https://github.com/paichart/paichart/blob/main/tutorials/02-the-ten-gold-standards.md)
- For pAIchart-specific implementation reference: `mcp-tool-gold-standard-pattern.md` (this knowledge base)
...
**The standard**: schema enforcement happens at the dispatch boundary (the router that maps action name to handler), not in upstream `validateRequest()` alone, and not in each handler. One place, looked up by action name:
...
**Success criteria**:
- The dispatch boundary (router) explicitly invokes the schema for every action it routes
- Both transport entries (HTTP + MCP) hit the same dispatch boundary
- A grep for `safeParse` / `parse` finds the call inside the dispatcher
- Adding a new action only requires registering it in the schema map — the dispatcher picks it up by name

**Failure modes**:
- Schema defined but the dispatch boundary doesn't invoke it
- HTTP path validates, MCP transport path bypasses validation
- Each handler invokes its own validation (inconsistent coverage; new handler ships without it)
- "Strict mode" and `.refine()` guards are present in the schema but never fire because nothing calls `safeParse`
...
| Handler | The action-specific function | Business logic only; assumes parameters are already schema-validated. |

The dispatch boundary is the load-bearing layer. The transport entry is too early (no per-action context); the handler is too late (no single audit point).
...
- The field has a `grade` (categorical) AND a `message` (explanation)
...
**Real-world example**: An MCP Hub `registry(action: "tools", service_name: "...")` handler returns a `qualityAssessment` graded against the registered service's schema quality. A service with full `inputSchema` for every tool gets grade A; one with name-only entries gets grade C with explicit upgrade guidance. The grade is computed from observable input (tools array shape), not a static label.

**Where this came from**: Surfaced during the 2026-05-22 MCP Hub Domain Testing pilot. Recognized as a Gold-Standard candidate worth promoting to tools beyond the hub — particularly for tools whose result quality varies with caller-supplied input.
...
The original thirteen standards were extracted from a production audit of a 28-tool MCP server (December 2025). The audit scored every tool on description quality, error handling, and response shape; the patterns this spec documents are what the highest-scoring tools had in common. Subsequent additions emerged from cleanup work and bug-class triage in early-to-mid 2026 (GS11–13 cross-cutting rule from the Zod-validation work), Wave 7 architectural review (GS14 — dispatch-boundary schema enforcement, May 2026), and the MCP Hub Domain Testing pilot (GS15 — self-grading tool responses, May 22, 2026).

For pAIchart-specific implementation guidance — file paths, code references, the dispatcher pattern that ties this surface together — see the companion document `mcp-tool-gold-standard-pattern.md` in this same knowledge base.
