...
> **Source:** pAIchart network-provisioning pipeline, 2026-07-01 (neutral-role retrofit validation run).
...
> **Device:** harvested **live, read-only** from the 2-node Arista cEOS lab (ceos1/ceos2, EOS 4.32.2.1F) via the isolated `nornir-mcp` container over the cloudflared tunnel.
> **Companion to** `example-change-report.md` (the Loopback0+BGP example). This run harvested the **evolved** state — the Loopback0 + BGP change already applied — and designed **PTP boundary-clock** (a non-VLAN/non-routing domain), validating the neutral `infra_*`-role retrofit + the technology-even protocol end-to-end on a real device. The Appendix shows the running-config it was designed against.
