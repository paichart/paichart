...
grep -c "computeDerivationContainmentFact\|computeDialectLintFact\|computeContractPropagationFact\|computeMarkerPresence" lib/services/execution-core.ts   # expect 9 — the ONE call site file. Imports plus call/comment references for all four facts. There is no registry loop yet, so each net is wired by hand and a new one can be forgotten
...
grep -c "computeRollbackContainmentFact\|hoistRollbackContainment" lib/services/execution-core.ts   # expect 3 — net #3 is wired at TWO points, which is the shape the registry's appliesTo must support: computed at the AUTHOR LEAF's persist (so the chainer carries it into the Reviewer's §6 — a SYNTHESIZE stamp lands after the review, which is the blindness the net exists to end) and HOISTED, never recomputed, at the leg SYNTHESIZE
```
...
grep -rln "pickResultJsonSummary" lib/ scripts/ | wc -l   # expect 7 — re-measured 2026-09-11 (+1: rollback-containment-enrichment.ts's E3b note) — every file that hoists the summary. Any new consumer must be added deliberately; the hoist is where a nesting mistake becomes invisible
```
...
grep -c "dialectLint\|contractPropagation" lib/mcp/server/tools/advanced/lean-card-facts.js   # expect 0 — ⚠️ KNOWN GAP, measured 2026-09-11: both facts are stamped and whitelisted but render on NO card line. Whether that is correct (neither is a gate conjunct today) or the A1/F7 seam defect repeating is an OPEN question for the coordinator — see the specialist's Open Questions. If this becomes non-zero, the gap was closed and this expectation must be re-measured. ⚠️ AND: nothing may NAME those two identifiers in that file's PROSE. A comment mentioning them makes this grep non-zero while the gap is still open — a checking tool made to lie by a comment about it. It happened on 2026-09-11 during the net #3 build and was caught by re-running this grep before commit; the comment now describes them without naming them
...
```bash
ls scripts/replay-*.ts | wc -l   # expect 4 — re-measured 2026-09-11 (+replay-rollback-containment; the others are replay-containment, replay-dialect-lint, replay-contract-propagation). Each runs the SHIPPING enrichment against a completed leg, read-only, in seconds. A fourth net must ship with its own runner (or consume the shared one, once the registry exists)
```
...
```bash
grep -c "contractApplicability" lib/services/execution-core.ts   # expect 4 — computed ONCE at the call site and nested on BOTH facts. NEITHER fact owns the predicate: putting it inside one would force the other to re-derive it (the two-extractor class). Nested, so no RESULT_JSON_SUMMARY_KEYS change — it rides inside two already-whitelisted facts (E3b-3 pins both)
...
```bash
grep -rn "MECHANICAL_NETS\|netRegistry\|registerNet" lib/ scripts/ | wc -l   # expect 0 — the shared net registry does NOT exist yet. Four facts are wired by hand at one call site. Rule of three fired (panel brief item 5, r19 follow-up); whichever of rollbackContainment / H-4 markerPresence extension lands first extracts it. Non-zero ⇒ the registry shipped and this discovery, the toolkit and the specialist config all need the same-commit update
grep -rln "rollbackContainment" lib/agents/harness/ | wc -l   # expect 4 — ✅ THE TRIPWIRE FIRED AND THE BUILD LANDED (stage 2a, 2026-09-11). Its former text carried a zero-expectation and promised that a non-zero result meant net #3 had shipped and sections A-D needed re-measuring; it did, they were, and both were corrected in the same commit. (The old expectation is DESCRIBED rather than quoted: a literal zero-expectation inside prose is read by audit-discovery-greps.sh as a live expectation, which made this very line report a false REGRESSION on its first pass — the third time in this file that a documented grep has predicted its own obsolescence and the audit caught the wording.) The four files: rollback-containment-enrichment.ts (producer), render-rollback-containment.ts (the §6 render) + render-pipeline-context.ts (its call site, reading the field off each chainedFrom entry) added by H3, and context-chainer.ts (the carry) added by the 2a-plumbing commit — which is the fourth H3 predicted here before it landed. The pure module names its types RollbackContainment* and does not carry the camelCase field name, so it does not count. THE REGISTRY IS STILL UNBUILT — stage 2b, held for a separate go — so the registry tripwire above is unchanged and the hand-wired sites are now SIX
```
...
That inversion is itself an open design question: `adding-a-containment-kind-toolkit.md` Step 0 is
...
The execution toolkit is `.claude/knowledge/pipelines/adding-a-containment-kind-toolkit.md` — Step 0
...
It is scoped to containment kinds today and is scheduled to be **generalised to adding-a-net** in
the same arc that extracts the shared registry. Until then, read it as the nearest procedure and
...
```bash
grep -c "STAGE_CHILD_SCAN_CAP\|CHILD_SCAN_CAP" lib/agents/harness/*.ts | grep -v ":0" | wc -l   # expect 4 — measured 2026-09-11: contract-propagation, rollback-containment, derivation-containment and dialect-lint enrichments. FOUR files holding the same constant for the same query is a registry candidate (stage 2b owns it), not a reason to add import edges between enrichments
```
