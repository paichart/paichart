...
## 🆕 2026-09-11 — fact PRODUCTION moved to execution-facts-discovery.md (§3b split)

The 2026-08-16 cross-port ①, 2026-08-23 dialect-lint and 2026-07-17 derivation-containment blocks —
and every proven grep in them — MOVED to `.claude/knowledge/discoveries/execution-facts-discovery.md`
(paired with `execution-facts-specialist`, owner of fact production since 2026-09-11). Run that
discovery when the question is how a fact is produced, stamped, whitelisted or rendered. This
discovery keeps how facts are CONSUMED: gates, `programReleasable`, verdict wiring, protocol
semantics. The fact schema is the contract between the two.
...
**Why**: `ROLE_GUIDANCE_LIBRARY` keys are SHARED ACROSS DOMAINS. Two keys each back four templates
(observability-config joined network-provisioning, terraform-iac and kubernetes-gitops on 2026-09-08),
so any edit is a four-domain edit — and nothing at the call site used to say so. This phase exists
...
```bash
# Which templates share each role key? (expect config_change_author 4, change_reviewer 4)
...
## containmentDisposition delivery — MOVED 2026-09-11 (§3b split)

The whitelist/E3b delivery greps and the `cKind` consumed-kind grep moved to
`.claude/knowledge/discoveries/execution-facts-discovery.md` (owner: `execution-facts-specialist`).
What stays yours: `containmentDisposition` is a gate input, and ABSENT means blocking.

## Duplicate-halt cone (added 2026-08-12; the consumed-kind grep moved to execution-facts-discovery 2026-09-11)
