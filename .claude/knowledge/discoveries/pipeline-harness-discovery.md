...
grep -c "## Derived Values" scripts/seed-protocol-prompts.ts                # expect 10 — (+1 2026-09-09: the marker clause now names the heading form the parser accepts) — MEASURED 2026-08-16 (was 5): network Phase-1 contract refs + pov-program taxonomy refs + the terraform-iac v1.2.0 port (Derivation-evidence section + bullets)
...
grep -c "## Harvested Allocations" scripts/seed-protocol-prompts.ts         # expect 13 — (+1 2026-09-09: marker clause) — MEASURED 2026-08-16 (was 8): +4 from the terraform-iac v1.2.0 Derivation-evidence port (section + phase bullets). The marker is now a CROSS-DOMAIN contract, no longer network-only
