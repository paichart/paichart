...
- **Shared naming/tags (BOTH domains)**: every change entry tagged `tf-lab-two-member`; change
  window "lab — any time".
...
- Each change package must include deterministic validation with expected outputs (network: exact
  ...
  - **"Deterministic" means a reviewer can run it and compare, without judgement.** Every validation
    ...
    could disagree on whether it passed. Write `show ip interface brief | include Loopback24` and the
