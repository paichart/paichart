...
- Iteration: IGP-T1 R7 (four migration phases, **SEQUENCED**, gate + apply + re-harvest loop) · 2026-08-24
- Topology: **2 nodes, 1 link** (the 3-node triangle was authored then SHELVED — it does not fit the
  prod host, and cEOS does not boot on the available off-host VM; see topology.json `_topologyNote`)

> **What this program is.** Migrate the IGP of a live 2-node brownfield network (OSPF area 0) to
...
> never writes to a device. Legs are migration PHASES, not devices — every leg spans BOTH switches.
...
- `targetProtocol`: IS-IS, **flat Level-2-only** (single area 0.0.0.0 maps to one L2 domain), one
  IS-IS instance with an **identical instance identifier on both devices**.
...
- `metricStyle`: wide. Per-interface IS-IS metrics translated from the harvested OSPF interface
  ...
  ⚠️ **Scope note, honest about what this topology can and cannot prove:** with two devices and one
  link there is no alternate path, so the stronger property — *every relative path PREFERENCE OSPF
  expresses is preserved* — is not exercised here. A clean pass proves faithful per-interface metric
  translation, NOT path-selection preservation. Do not report it as the latter.
...
- **P3 — preference shift.** Change package flipping preference to IS-IS via the contract's
  single-knob mechanism, identically on both devices. Validation: exact commands + expected
...
- **P4 — OSPF removal.** Change package removing the OSPF process from both devices. **The
...
- The IS-IS instance identifier is identical on both devices (convention; any consistent value).
