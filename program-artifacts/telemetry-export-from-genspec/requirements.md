...
- Origin: authored by the `requirements-authoring-protocol` v1.0.0 pipeline (run 20260921-1041) in
  POV *Requirements Generator — Validation*, phase *1 — Generate Against Ground Truth*; publish-time
  pass 2026-09-22. ⚠️ That is where this document was WRITTEN, not where the program it describes
  runs — the running POV and phase are set by the person who launches it.
- Control: `program-artifacts/telemetry-export-four-domain` — same objective, same gates, and a
  `topology.json` byte-identical to this one. The requirements document is the only variable.
