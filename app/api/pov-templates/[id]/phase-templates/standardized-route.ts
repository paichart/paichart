...
async function updateTemplateWithPhaseTemplates(templateId: string, phaseTemplateIds: string[]): Promise<any> {
  try {
    ...
    povLogger.info({ templateId }, 'Template updated with phase templates');
...
export async function GET(
  ...
  try {
    ...
    povLogger.error({ err: error, povTemplateId }, 'GET phase templates API error');
    return NextResponse.json(
  ...
  try {
    ...
    try {
      ...
      if (!Array.isArray(phaseTemplateIds)) {
        povLogger.warn({ povTemplateId }, 'phaseTemplateIds is not an array');
        return NextResponse.json(
      ...
      povLogger.error({ err: parseError, povTemplateId }, 'Failed to parse phase template request body');
      return NextResponse.json(
    ...
    povLogger.error({ err: error, povTemplateId }, 'POST phase templates API error');
    return NextResponse.json(
