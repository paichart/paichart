...
| Your own sales theatres / countries / regions | a default set is seeded (4 theatres, 17 countries); `Settings → Geographical` is per-user defaults for new POVs, not an editor. Add countries/regions by editing `scripts/seed-geographical-data.js` and re-running it (adds missing rows; never renames or deletes). A new theatre is a schema change (`SalesTheatre` enum → `npx prisma db push`) — `.claude/knowledge/guides/GEOGRAPHICAL_DATA_MANAGEMENT.md` |
