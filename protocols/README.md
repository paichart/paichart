...
| [`kubernetes-gitops`](kubernetes-gitops-protocol.md) | 1.3.0 | Declarative GitOps change packages; offline validation only | v1.2.0's baseline-scoped drift clause ships **seeded-unvalidated** — no published round has presented out-of-band drift yet. We say so here for the same reason the pack publishes failed rounds |
| [`observability-config`](observability-config-protocol.md) | 1.0.0 | Observability change packages (Prometheus rules / OTel collector / Grafana provisioning) — witnessed-artifact taxonomy, unconditional live-stack Phase 0, per-class rollback provenance | Seeded 2026-09-10; **not yet exercised by a published round** — stated here for the same reason the pack publishes failed rounds |
