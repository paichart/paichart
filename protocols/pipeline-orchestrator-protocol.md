...
2. **No duplicate pipeline.** Call `project(action: "pov.details", povId: "<your pov id>")` and read the stage list for YOUR phase from its `phases[].stages[]` (there is no `stage.list` action — `project` exposes pov.list / pov.details / task.list / task.context). If a stage in this phase is already named "Pipeline: X" for a similar objective, STOP — UNLESS your task carries an explicit clearance (next paragraph). Post a comment linking the existing pipeline and ask the human whether to proceed. Do NOT silently duplicate work.
   ...
   **The human's answer arrives in TASK STATE, never in comments** (comments are history you rightly do not trust — a comment reply cannot clear this check). Before stopping, check for an explicit clearance **bound to the specific duplicate you found**: canonically `metadata.duplicateAcknowledged` set to the existing stage's id or exact name — or, where more than one prior run exists, a LIST naming each of them; or, as the human-readable backup, a clearly delimited trailing block in your task DESCRIPTION beginning `PRE-FLIGHT CLEARANCE:` that names each such stage. Treat this check as RESOLVED **only when EVERY duplicate you detected is named** — a clearance naming a different stage, naming only some of the duplicates you found, or naming none is stale/unbound and does NOT clear the check; note the resolution in your mode comment and proceed with CREATE. If no matching clearance exists, stop as above; the human releases you by adding the clearance (`task.update` metadata or description — never a comment) and re-executing you. (Live incident 2026-07-15: a correct duplicate-stop could not be released by comment replies — two re-runs re-stalled until the clearance moved into task state.)
...
- **Never retype a value a child already receives on its own channel — and never hand a child a check the platform performs.** A value chained from an upstream leg reaches your Architect and Author WITH its provenance, in their chained context; your reviewer is deliberately NOT given it. A copy you type into a brief carries no provenance, and a reviewer shown one has cited it as "independent" corroboration of the very value it copies (live 2026-09-24; measured 2026-09-25: a chained value retyped into the briefs of 29 of 45 consuming legs, into all three children in 23 of them). Name the channel instead ("consume exactly the value chained from the upstream leg"). Whether a consumed value EQUALS what upstream derived is checked by the platform (the consumed-value check at synthesis), so split that requirement by who can discharge it: the Author's brief carries the constraint (consume exactly the chained value, and declare it the way the active domain protocol requires); the reviewer's brief carries only what the reviewer can verify from the package in front of it — that the package APPLIES exactly the value it DECLARES. Never write "confirm it matches exactly what was chained" into a reviewer's brief: the reviewer cannot see the chained value, so the instruction sends it looking for one somewhere else.
- **Descriptions state the objective and carry forward the plan's/requirements' own constraints — verbatim (a requirement the PLATFORM checks is the one exception — route it by who can discharge it, per the bullet above). Do NOT compose new acceptance criteria, thresholds, or verification gates that the active protocol or the requirements artifact does not state**: an invented constraint becomes a rule owned by nobody, and over-constraint is how a run reaches a false "impossible" (2026-08-11: a harness-invented "verify no /31 or /30 widening" gate — present in no protocol and no requirements — made a child reject a valid selection).
- **A named target scopes the CHANGE, never the HARVEST. Descriptions may NOT narrow a Phase 0 brief below what the active protocol already puts in scope.** A target named in the objective or the interface contract — a workload, a resource address, an interface, a scrape job — says what the change will TOUCH. It does not say what the change must be JUSTIFIED against, and **Phase 0 finishes before anyone chooses the design**, so evidence not asked for at Phase 0 cannot be recovered inside the leg. The protocol's Phase 0 section decides what an objective's CLASS requires; your brief carries that forward and may add to it, never subtract from it. ⚠️ **Never write "list every X in scope" without stating what the scope IS** — a brief whose scope is the named target is a closed loop that can only return the target. Live 2026-09-23: a brief narrowed "the objective" to the single named workload it mentioned, the harvester executed that faithfully, and the design one phase later applied to the whole population containing it — with nothing in the harvest able to bound what it reached. The reviewer caught it and the gate refused — correctly, and three phases too late to fix.
...
- `"approved"` —
  ...
  - **Pipeline with NO reviewer child** (`reviewerPresent: false`; score = the LOWEST child confidence; legitimate ONLY when the active protocol does not mandate one — see the roster-defect rule below): every child is terminal (`status = COMPLETED`) with its authoritative execution SUCCESS and no child `executionStatus = FAILED`, AND no child's `result.json` carries an anti-fabrication trust signal (`SILENT_REFUSAL`, `TOOL_LOOP_DEGRADED`, `PROTOCOL_STEP_SKIPPED`, `TEMPLATE_MISMATCH_SELF_REPORTED`, `BUDGET_EXHAUSTED`, `TOOL_FAILURES`, `TRUNCATED_PARTIAL_OUTPUT`), AND no `derivationContainment` violation where that fact is present. With no reviewer reading the deliverable, these mechanical trust FACTS are the fabrication catch — gate inputs here, not advisory. **Why THESE seven and not every category** (2026-09-16; seventh added 2026-09-24): the list is the OUTPUT-EXISTS-BUT-MAY-BE-UNBACKED set — a deliverable WAS produced and something makes it untrustworthy. `TRUNCATED_PARTIAL_OUTPUT` is the plainest member: the response was cut off by the output-token ceiling after text was written, so the deliverable is missing its TAIL — often the part that carried its conclusion, its machine-read blocks or its confidence line (two live incidents, 2026-07-16 and 2026-09-24). Deliberately EXCLUDED because they mean no deliverable was produced at all, so they carry no fabrication risk: EMPTY_DELIVERABLE, TRUNCATED_NO_OUTPUT, HARNESS_NO_OUTPUT (each fails the leg by other means), and USER_CONFIG_REQUIRED (a configuration fault, not a trust signal). TOOL_FAILURES was ADDED as the direct sibling of TOOL_LOOP_DEGRADED — both mean output exists over failed tools — and in candour it has NEVER FIRED (0 of 214 archived degradations), so this is correctness by symmetry, not a response to evidence. ⚠️ TEMPLATE_SCOPE_MISMATCH is RETIRED (2026-07-17, ~60 firings / 0 true positives) yet is 73% of all archived errorCategory hits: anyone corpus-measuring this field must exclude it or over-count threefold. This is a "ran clean, no QA gate" approval, NOT a reviewed one — `reviewerPresent: false` records that.
