...
| VT-20 | `tests/VT-20-dialect-lint-first-catch-and-the-delivery-defect.md` | ✅ **VERIFIED (2026-08-26)** — the mechanical `dialect-lint` net, on its FIRST live round, caught two absent canonical config lines in a package its own LLM reviewer approved at 86/100 with zero blocking issues; impact proven on-device (the config enters, commits and displays while the protocol stays DISABLED). The forensics behind it are the second claim: an ABSENT guard and a DISOBEYED guard produce identical evidence and demand opposite fixes. Measured — short NEGATIVE rules did reach the agents (so VT-19's prose-bypass finding stands), the long POSITIVE exemplar did not (3 of 10 lines survived), and the interface contract had never reached ANY leg's children (7 of 7 legs, 0 of N children), making the reviewer's mandated transcription check an unsatisfiable predicate. Fixed and live-verified on production |
