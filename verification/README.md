...
| VT-17 | `tests/VT-17-second-program-protocol.md` | ✅ **PASSED (6/6, 2026-08-18)** — a SECOND program protocol went from GUI-authored DRAFT to planning and spawning a real program with ZERO code change: token → canonical stamp → **the tier-split's first live firing** (a DRAFT program row hard-fails by name with the fix in the error text, never a silently-fabricated base-only "program") → ACTIVE → composed PLAN + full PLAN-SPAWN roster, byte-identical preamble across executions, zero leakage of the original protocol's identity. En route the copied duplicate pre-flight and its clearance channel worked under a binding that did not exist when they were written |
| VT-18 | `tests/VT-18-firewall-a3-remediation-campaign.md` | ✅ **PASSED (Round 5 of 5, 2026-08-22)** — a five-round remediation campaign reached an EARNED green: `programReleasable: true` with all three legs approved first-pass (90/90/87), Node C `## VERDICT: APPROVED` 0-blocking, the derived pool `10.99.0.6/31` exactly minimal for that rebuild's scatter, mechanically verified at derivation and consumed verbatim at both downstream hops, chained coverage 3/3 with 0 degraded. Zero retries, zero human overrides — the only human actions were gate reviews |
| VT-19 | `tests/VT-19-igp-migration-applied-state-loop.md` | ✅ **PASSED (R10 of 10, 2026-08-23/25)** — the S2 *applied-state* interdependency flavor, where each gate is a human APPLY to live devices and the next leg designs against RE-HARVESTED state rather than the predecessor's declared output. On R10 all four migration phases were applied and verified on two live Arista cEOS nodes (coexistence → parity → preference shift → OSPF removal, persisted to startup-config) **and the program then correctly REFUSED to certify itself.** Both halves are the claim |
| VT-20 | `tests/VT-20-dialect-lint-first-catch-and-the-delivery-defect.md` | ✅ **VERIFIED (2026-08-26, updated 2026-08-27)** — the mechanical `dialect-lint` net caught two absent canonical config lines in a package its own LLM reviewer approved at 86/100; impact proven on-device (config enters, commits and displays while the protocol stays DISABLED). Root cause behind it: the interface contract had never reached ANY leg's children (7 of 7 legs, 0 of N children), making the reviewer's mandated transcription check an unsatisfiable predicate. **Follow-through on the next full round (R12): four packages applied VERBATIM to live devices with ZERO config defects and ZERO device rejections, 4 of 4 children holding the contract — delivery alone fixed transcription, and the proposed deterministic renderer was dropped unbuilt.** The update also qualifies claim 1 honestly: the checker's presence half was shape-dependent and worked here partly by luck, and a corpus measurement found it produced a FALSE positive on a removal leg where the prose reviewer was right. Both defects fixed and pinned |
| VT-21 | `tests/VT-21-provenance-fact-replaces-an-unmakeable-judgement.md` | ✅ **VERIFIED (2026-09-11)** — three CORRECT change packages were refused on provenance grounds by reviewers that structurally cannot see the harvest (each said so in its own verdict, then asserted the conclusion anyway; measured base rate of true fabrication across the archive: **0 of 56**). Prose closed one lane and the class reappeared in the next. A mechanical containment fact, stamped where the evidence lives and delivered into the reviewer's context, cleared it — **26 of 26 restore lines found verbatim**, verdict APPROVED, and the reviewer kept package-completeness as its own judgement rather than reading a green fact as approval. Scoped, non-blocking, one lane — the limits are stated in the doc |
| VT-22 | `tests/VT-22-a-slot-that-demanded-a-network-observable-becomes-a-declarable-property.md` | ✅ **VERIFIED (2026-09-12)** — a shared prompt slot asked all four domains for a *maintenance-window note*, an observable only one of them has. Measured across the production archive, **44 of 49** non-network packages invented a meaning for it (k8s 5/5, terraform 31/33, observability 8/11) — never wrong enough to block anything, never checkable. Naming the underlying property instead (what the apply requires, what it disrupts while running, the first post-apply check) and making the absence **declarable** produced **zero invented windows across four live runs**; three domains declared no-window-needed with a domain-grounded reason, no reviewer demanded a window, and the control domain still produced window guidance. Also recorded: a quota exhaustion voided one leg, and the harvester, architect, author and reviewer each refused to proceed rather than fabricate a baseline. **Repeated independently on the devext self-host** running the published open-source build: three of four domains reproduced, two converging on the clause's own phrase *"declared, not invented"* without either run seeing the other — **zero invented windows across seven uncontaminated packages on two installs**. The self-host's network control is VOID and says so: its phase description had been written to record the expected answer and the package cited it back, which is the obligation being hand-carried — the production round is clean by the same check. The weak control case, the contamination, and the one-run-per-domain limit are all stated in the doc |
| VT-23 | `tests/VT-23-a-permission-that-reaches-the-party-who-blocks.md` | ✅ **VERIFIED (2026-09-14/15)** — a permission earned on 2026-08-27 sat in a bullet addressed to the AUTHOR, while the reviewer read its own bullet (silent), then a role-neutral clause saying *literal or drop*, then role guidance telling it to block non-fact validation. Assembled-prompt evidence: the network reviewer received the sanctioned shape, the terraform reviewer received **nothing**. So the contradiction resolved against the permission every time — **the fix did not prevent its own incident**, which recurred three weeks later in another domain. Measured first: **0 of 21** network harvests contain raw CLI, and **73 of 110** lines inside literal expected-output blocks appear in no harvest, i.e. most "literal" content was prediction — four such predictions had already shipped wrong, one propagating a bad number into a blocking defect. One shared clause now carries the rule to all four domains (source-level extraction; the four published files stay byte-identical with the rule inline) and addresses the reviewer directly. **Both arms tested**: a live cEOS leg whose author wrote six presence assertions was APPROVED at 90 citing *"the sanctioned presence-assertion shape"* and checking all four required parts; a control probe carrying the SAME well-formed shape into terraform — which sanctions none — was BLOCKED as *"unsanctioned"*, with the REPLACE remedy prescribed first. Identical artifact, opposite verdicts, each citing its own domain's licensing state. A blanket permission would have approved both; the pre-change state blocked both. Honest limits stated: arm two is a control probe rather than a natural event (base rate 1 in 38), and whether the clause RAISES non-literal usage where nothing is licensed is a rate question one run cannot answer — pre-change baselines are recorded for the quarterly re-measure |
| VT-24 | `tests/VT-24-a-self-reported-correction-does-not-satisfy-a-gate.md` | ✅ **VERIFIED (2026-09-16)** — a runtime value crossed a DOMAIN boundary verbatim (Kubernetes pod range → Terraform S3 policy, no shared vocabulary); a leg whose evidence failed machine-parse was refused by its own reviewer and approved after correction; and the program gate then refused a self-reported "outcome corrected" claim, followed the mechanical record, and **failed CLOSED**. Includes a recorded correction: the author first claimed the blocking fact was stale — investigation proved it CORRECT (the derivation had no harvest pool to be checked against, a protocol coverage gap), which strengthens the claim rather than weakening it. |
