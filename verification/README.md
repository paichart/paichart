...
| VT-13 | `tests/VT-13-containment-conjunct-blocking-direction.md` | 🟡 **STILL OPEN after Runs 16 and 17** — the derivation conjunct BLOCKS and the block is attributable to it. Both ran Branch B (no violation), so the claim is unverified and neither is recorded as a pass. Between them: four first-time live proofs — CC3's edge carry, the harvestedCount fact deciding a run, `derivedValues` crossing the edge, and acceptance check 1 verified mechanically end to end |
