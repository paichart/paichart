...
| VT-13 | `tests/VT-13-containment-conjunct-blocking-direction.md` | 🟡 **STILL OPEN after Run 16** — the derivation conjunct BLOCKS and the block is attributable to it. Ran Branch B (no violation occurred), so the claim is unverified and Run 16 is NOT recorded as a pass. Did produce three first-time live proofs, and found that a numbered requirements check is not binding on a reviewer |
