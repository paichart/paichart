...
if not PROGRAM_TASK:
    print("usage: kind-inject.py <programRootTaskId> [--dry-run] [--kind=asn] [--value=65100] [--device=ceos1]", flush=True)
    sys.exit(2)
# Defaults reproduce the VT-13 asn round exactly; --kind/--value retarget it without a second copy.
# A forked injector is a copy that drifts, and the Run-20/21 defects were both in the shared helper —
# fixing one fork would have left the other broken.
INJECT_KIND = _flag("kind", "asn")
INJECT_ASN = _flag("value", "65100")   # see module docstring — chosen to fire exactly one class
INJECT_DEVICE = _flag("device", "ceos1")
    ...
    if rows:
        ...
        if st and st[0][0] == "0":
            ...
            try:
                ...
                sys.exit(2)
            arr.append({"kind": INJECT_KIND, "value": INJECT_ASN, "device": INJECT_DEVICE})
            ...
            if DRY:
                ...
                # Verify the entry WE would add is present, keyed on the ACTUAL kind — this check
                # hardcoded 'asn' and reported False for every other kind while still exiting 0,
                # i.e. a self-check that could not fail for the case it was generalised to serve.
                injected_ok = any(str(e.get('kind')) == INJECT_KIND and str(e.get('value')) == str(INJECT_ASN)
                                  for e in (back or []))
                print(f"[asn] DRY RUN OK — would inject into artifact {aid} on task {task_id}\n"
                      ...
                      flush=True)
                # Gate the exit on the round-trip AND on the injected entry being found. Previously
                # only the length was checked, so a dry run could print `injected: False` and exit 0.
                sys.exit(0 if back and len(back) == len(arr) and injected_ok else 2)
            ...
            print(f"[asn] injected kind={INJECT_KIND} value={INJECT_ASN} device={INJECT_DEVICE}. "
                  f"Read the outcome against the VT's committed observables — do NOT restate an "
                  f"expectation here, and do not treat this line as evidence of one.", flush=True)
            sys.exit(0)
        else:
