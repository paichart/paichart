...
} from '../lib/agents/harness/derivation-containment';
...
test('FINDING-F pin (run 10, 2026-07-18): enrichment checks derivation-EXISTENCE before the harvest anchor — a leg emitting NEITHER block reads benign no-derived-values-block, not blocking harvest-block-missing', () => {
  // Path follows the 2026-07-30 extraction: the enrichment moved OUT of execution-core so it could
  // be replayed without a program run (scripts/replay-containment.ts). This test caught the move.
  const src = require('fs').readFileSync(require('path').join(__dirname, '../lib/agents/harness/derivation-containment-enrichment.ts'), 'utf-8');
  const iDerived = src.indexOf('if (!derived) {');
  const iHarvested = src.indexOf('} else if (!harvested) {');
  assert(iDerived > -1 && iHarvested > -1, 'ordering branches missing from the enrichment module');
  assert(iDerived < iHarvested,
    'ORDER REGRESSION: !harvested checked before !derived — a leg that emits NEITHER block would be stamped blocking harvest-block-missing-or-unparseable instead of the benign no-derived-values-block (finding (f), run-10 false-block class)');
});
